...
const getShippingCosts = (precio, restaurant) => {
  if (precio <= 10) {
...
const update = async function (req, res) {
  ...
  try {
    ...
    const restaurant = await Restaurant.findByPk(pedidoOriginal.restaurantId)
    ...
    UpdatedData.shippingCosts = getShippingCosts(precio, restaurant)
    UpdatedData.price = precio + UpdatedData.shippingCosts

    await Order.update(UpdatedData, { where: { id: req.params.orderId }, transaction })
    ...
    const returned = await Order.findOne(
      {
