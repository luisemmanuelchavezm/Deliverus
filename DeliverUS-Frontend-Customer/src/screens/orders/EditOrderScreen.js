...
export default function RestaurantDetailScreen ({ navigation, route }) {
  ...
  const fetchOrderQuantity = () => {
    ...
    if (order && Array.isArray(order.products)) {
      order.products.forEach(product => {
        productQuantityMap.set(product.id, product.OrderProducts.quantity)
      })
    }
  ...
  const createOrder = async () => {
    try {
      const orderData = { address, products: [...productQuantity].filter(([_, quantity]) => quantity > 0).map(([productId, quantity]) => ({ productId, quantity })) }
      ...
      showMessage({
        message: `Problems while updating order: ${error}`,
        type: 'danger',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
